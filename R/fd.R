...
#' @export
#'
#' @details Calls function [stats::spec.pgram()] to estimate the scaling exponent of a timeseries based on the periodogram frequency spectrum. After detrending and normalizing the signal (if requested), [stats::spec.pgram()] is called using a cosine taper = 0.5 and it will automatically pad the data (argument `fast = TRUE`).
#'
...
fd_psd <- function(y,
  ...
  if(N==npad){npad <- 0}
  psd  <- stats::spec.pgram(y, fast = TRUE, demean=FALSE, detrend=FALSE, plot=FALSE, taper=0.5)
  ...
  tsData <- data.frame(y=c(as.numeric(y_ori),as.numeric(y)),
    ...
    logSlopes <- data.frame(size = c(outList$PLAW$size[1:NROW(outList[[2]]$fitlm1$fitted.values)],
                                     outList$PLAW$size[1:NROW(outList[[3]]$fitlm2$fitted.values)]),
                            bulk = c(outList$PLAW$bulk[1:NROW(outList[[2]]$fitlm1$fitted.values)],
  ...
  #g2 <- plotFD_loglog(outList, title = title)

  g2 <- plotFA_loglog(outList,
                      title = title,
                      subtitle = paste0("Full range: Slope = ",round(outList$fullRange$sap,digits=2)," | Hasselman's informed FD = ",round(outList$fullRange$FD,digits = 2),"\nHurvich-Deo: Slope = ",round(outList$fitRange$sap,digits = 2)," | Hasselman's informed FD = ",round(outList$fitRange$FD,digits = 2)),
                      xlabel = "Frequencies",
                      ylabel = "Power",
                      logBase = "e")
  ...
  outList$tsdata <- tsData

 # outList$logSlopes <- logSlopes
  ...
  return(outList[c(returnPLAW,TRUE,TRUE,returnInfo,returnPlot, TRUE)])
}
...
fd_sda <- function(y,
  ...
  #g2 <- plotFD_loglog(fd.OUT = outList, title = title, subtitle = tsName, logBase = "e", ylabel = "Standardised Dispersion")
