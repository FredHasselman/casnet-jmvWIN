...
plotFA_loglog <- function(fd.OUT,title="log-log regresion",subtitle="",xlabel="Bin size",ylabel="Fluctuation",logBase=c("e","2","10")[1]){
  ...
  g <- ggplot2::ggplot(data.frame(fd.OUT$PLAW), ggplot2::aes_(x= ~size, y=~bulk), na.rm=TRUE) +
    ggplot2::geom_point() +
    ggplot2::geom_smooth(data = logSlopes,  ggplot2::aes_(x=~x, y=~y, colour = ~Method, fill = ~Method), method="lm", formula = y~x, alpha = .2) +
...
plotFD_loglog <- function(fd.OUT, title="", subtitle="", xlabel="Bin size", ylabel="Fluctuation", logBase=NA, doPlot = TRUE){
  ...
  if(logBase!="no"){
    g <- g +
      ggplot2::geom_smooth(data = logSlopes,  ggplot2::aes(x=x,y=y, colour = Method, fill = Method), method="lm", alpha = .2) +
      ggplot2::scale_x_continuous(name = paste0(xlabel," (",logFormat,")"),
                                  breaks = breaksX,
...
#'
plotDC_res <-  function(df_win, win, useVarNames = TRUE, colOrder = TRUE, useTimeVector = NA, timeStamp = "01-01-1999", doPlot = TRUE, title = '', resVariable = "Dynamic Complexity", subtitle = "", xlabel = "Time", ylabel = ""){
...
#' @family Dynamic Complexity functions
#'
plotDC_ccp <-  function(df_ccp, win, useVarNames = TRUE, colOrder = TRUE, useTimeVector = NA, timeStamp = "31-01-1999", doPlot = TRUE, title = '', subtitle = "", xlabel = "Time", ylabel = ""){
  ...
  colnames(df_ccp)[colnames(df_ccp)%in%"sig.peaks"]<-"Significant PEAKS"
